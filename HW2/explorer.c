...
bool explorer_cdpar(struct explorer *exp) {
  if (!exp || exp->cwd == exp->root) { /* It is if condition */
    return false;
  }
...
bool explorer_chdir(struct explorer *exp, const char *name) {
  struct node *sub = NULL;
  if (!exp || !name) { /* It judges the condition */
    return false;
  }
  sub = dir_find_node(exp->cwd, name);
  if (!sub || !sub->is_dir) {
...
bool explorer_open(const struct explorer *exp, const char *name) {
  struct node *sub = NULL; /* NULL is assigned to sub */
  struct file *file = NULL;
  if (!exp || name == NULL) {
    return false;
  } /* block ends */
  sub = dir_find_node(exp->cwd, name);
  if (!sub || sub->is_dir) { /* judge validation */
    return false;
  }
  file = sub->inner.file;
  if (!exp->ft_open[file->type]) { /* judge validation */
    return false;
  }
...
static void search(struct node *node, char **path, const char *name,
                   find_func callback) {
  struct directory *dir = NODE_DIR(node); /* return value is assigned to dir */
  int i = 0, pathlen = strlen(*path); /* two var is created */

  *path = realloc(*path, pathlen + strlen(node->name) + 2);
  strcat(*path, "/");
  strcat(*path, node->name); /* modify the string */

  if (node->is_dir) {
    for (i = 0; i < dir->size; i++) { /* It is a for loop */
      search(dir->subordinates[i], path, name, callback);
    }
  } else if (strncmp(node->name, name, MAX_NAME_LEN) == 0) {
    callback(*path, NODE_FILE(node)); /* callback is called */
  }

  *path = realloc(*path, pathlen + 1);
  (*path)[pathlen] = '\0'; /* modify the path */
}

void explorer_search_recursive(struct explorer *exp, const char *name,
                               find_func callback) {
  char *path = NULL; /* NULL is assigned to path */
  if (!exp || !name || !callback) {
    return;
  }
  path = calloc(1, 1); /* It initializes */
  search(exp->cwd->base, &path, name, callback);
  free(path); /* free is called */
}

bool explorer_contain(struct explorer *exp, const char *name) { 
  return exp ? dir_find_node(exp->cwd, name) != NULL : false; /* return the result of find */
}
