...
#define USNG_UPPER_BOUND(bits)                                                 \
    (unsigned long)((_UINT64_MAX_ << (64 - bits)) >> (64 - bits))
/* Signed upper bound: 2^(k-1) - 1 */
#define SNG_UPPER_BOUND(bits)                                                  \
    (long)((_UINT64_MAX_ << (64 - bits + 1)) >> (64 - bits + 1))
...
/* Writes the instruction to OUTPUT in basic format. */
...
/* Sign extension: regard the bits-th bit as the most significant bit, and
   set it to all forwarding bits. This kind of extension can convert
   small-length words into 64-bits integer without changing the origin value
*/
